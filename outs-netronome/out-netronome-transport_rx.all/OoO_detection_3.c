...
__forceinline
void __event___handler_OoO_DETECT_OoO_detection_3() {
  uint32_t v1;
  uint32_t v2;
  uint32_t v3;
  ...
  v4 = &work;
  v5 = &work_ref;
  cls_workq_add_thread(WORKQ_ID_OoO_DETECT_3, v5, sizeof(*v5));
  *(v4) = *(v5);
  v6 = v4->ctx;
  v7 = &v4->f0;
  v8 = v6->f0;
...
int main(void) {
	init_me_cam(16);
	init_recv_event_workq(WORKQ_ID_OoO_DETECT_3, workq_OoO_DETECT_3, WORKQ_TYPE_OoO_DETECT, WORKQ_SIZE_OoO_DETECT, 8);
	wait_global_start_();
	for (;;) {
		__event___handler_OoO_DETECT_OoO_detection_3();
	}
}
