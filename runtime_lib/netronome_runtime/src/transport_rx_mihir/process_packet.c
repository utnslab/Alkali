...
struct pkt_info_t _loc_buf_6;
__xrw struct pkt_info_t _loc_buf_6_xfer;
...
__forceinline
void __event___handler_NET_RECV_process_packet() {
  int64_t v1;
  int32_t v2;
  int32_t v3;
...
int main(void) {
	...
	wait_global_start_();
	for (;;) {
		__event___handler_NET_RECV_process_packet();
	}
}
