...
static struct firewall_ip_entries_t _loc_buf_18;
__xrw static struct firewall_ip_entries_t _loc_buf_18_xfer;
...
__forceinline
void __event___handler_NET_RECV_1_process_packet_1_2() {
  uint32_t v1;
  ...
  v2 = &work;
  v3 = &work_ref;
  cls_workq_add_thread(WORKQ_ID_NET_RECV_1_2, v3, sizeof(*v3));
  *(v2) = *(v3);
  v4 = v2->ctx;
  v5 = v2->f0;
  v6 = v2->f1;
...
int main(void) {
	init_me_cam(16);
	init_recv_event_workq(WORKQ_ID_NET_RECV_1_2, workq_NET_RECV_1_2, WORKQ_TYPE_NET_RECV_1, WORKQ_SIZE_NET_RECV_1, 8);
	wait_global_start_();
	for (;;) {
		__event___handler_NET_RECV_1_process_packet_1_2();
	}
}
