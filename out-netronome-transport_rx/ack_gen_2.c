#include "nfplib.h"
#include "prog_hdr.h"
#include "extern/extern_dma.h"
#include "extern/extern_net.h"

static struct repack_type_1 _loc_buf_33;
__xrw static struct repack_type_1 _loc_buf_33_xfer;
static struct eth_header_t_sub_0 _loc_buf_32;
__xrw static struct eth_header_t_sub_0 _loc_buf_32_xfer;
static struct tcp_header_t_sub_0 _loc_buf_31;
__xrw static struct tcp_header_t_sub_0 _loc_buf_31_xfer;
static struct ip_header_t_sub_1 _loc_buf_30;
__xrw static struct ip_header_t_sub_1 _loc_buf_30_xfer;
static int rr_ctr = 0;
__declspec(aligned(4)) struct event_param_ACK_GEN work;
__xrw struct event_param_ACK_GEN work_ref;
__declspec(aligned(4)) struct event_param_NET_SEND next_work_NET_SEND;
__xrw struct event_param_NET_SEND next_work_ref_NET_SEND;

__forceinline
void __event___handler_ACK_GEN_ack_gen_2() {
  uint16_t v1;
  uint32_t v2;
  __declspec(aligned(4)) struct event_param_ACK_GEN* v3;
  __xrw struct event_param_ACK_GEN* v4;
  __shared __cls struct context_chain_1_t* v5;
  struct ack_info_t* v6;
  __shared __cls struct ip_header_t_sub_1* v7;
  struct ip_header_t_sub_1* v8;
  __xrw struct ip_header_t_sub_1* v9;
  __shared __cls struct tcp_header_t_sub_0* v10;
  struct tcp_header_t_sub_0* v11;
  __xrw struct tcp_header_t_sub_0* v12;
  struct __buf_t v13;
  __shared __cls struct eth_header_t_sub_0* v14;
  struct eth_header_t_sub_0* v15;
  __xrw struct eth_header_t_sub_0* v16;
  uint48_t v17;
  uint48_t v18;
  struct eth_header_t_sub_0* v19;
  struct eth_header_t_sub_0* v20;
  uint32_t v21;
  uint32_t v22;
  struct ip_header_t_sub_1* v23;
  struct ip_header_t_sub_1* v24;
  uint16_t v25;
  uint16_t v26;
  struct tcp_header_t_sub_0* v27;
  struct tcp_header_t_sub_0* v28;
  uint32_t v29;
  struct tcp_header_t_sub_0* v30;
  uint32_t v31;
  struct tcp_header_t_sub_0* v32;
  __xrw struct eth_header_t_sub_0* v33;
  struct repack_type_1* v34;
  __xrw struct repack_type_1* v35;
  __xrw struct ip_header_t_sub_1* v36;
  __xrw struct tcp_header_t_sub_0* v37;
  __declspec(aligned(4)) struct event_param_NET_SEND* v38;
  v1 = 64;
  v2 = 0;
  v3 = &work;
  v4 = &work_ref;
  cls_workq_add_thread(WORKQ_ID_ACK_GEN_2, v4, sizeof(*v4));
  *(v3) = *(v4);
  v5 = v3->ctx;
  v6 = &v3->f0;
  v7 = &v5->f1;
  v8 = &_loc_buf_30;
  v9 = &_loc_buf_30_xfer;
  cls_read(&v9->f0, &v7->f0, 8);
  *(v8) = *(v9);
  v10 = &v5->f3;
  v11 = &_loc_buf_31;
  v12 = &_loc_buf_31_xfer;
  cls_read(&v12->f0, &v10->f0, 12);
  *(v11) = *(v12);
  v13 = v5->f0;
  v14 = &v5->f2;
  v15 = &_loc_buf_32;
  v16 = &_loc_buf_32_xfer;
  cls_read(&v16->f0, &v14->f0, 12);
  *(v15) = *(v16);
  v17 = v15->f0;
  v18 = v15->f1;
  v15->f0 = v18;
  v15->f1 = v17;
  v21 = v8->f0;
  v22 = v8->f1;
  v8->f0 = v22;
  v8->f1 = v21;
  v25 = v11->f0;
  v26 = v11->f1;
  v11->f0 = v26;
  v11->f1 = v25;
  v29 = v6->f0;
  v11->f2 = v29;
  v31 = v6->f1;
  v11->f3 = v31;
  v33 = &_loc_buf_32_xfer;
  *(v33) = *(v15);
  v13.offs = 0;
  mem_write32(&v33->f0, v13.buf + v13.offs, 12);
  v34 = &_loc_buf_33;
  v34->f0 = v1;
  v35 = &_loc_buf_33_xfer;
  *(v35) = *(v34);
  v13.offs = 16;
  mem_write8(&v35->f0, v13.buf + v13.offs, 2);
  v36 = &_loc_buf_30_xfer;
  *(v36) = *(v8);
  v13.offs = 26;
  mem_write32(&v36->f0, v13.buf + v13.offs, 8);
  v37 = &_loc_buf_31_xfer;
  *(v37) = *(v11);
  v13.offs = 38;
  mem_write32(&v37->f0, v13.buf + v13.offs, 12);
  v38 = &next_work_NET_SEND;
  v38->ctx = v5;
  v38->f0 = v13;
  next_work_NET_SEND.meta.len = next_work_NET_SEND.f0.sz;
  inlined_net_send(v38);
  return;
}


int main(void) {
	init_me_cam(16);
	init_recv_event_workq(WORKQ_ID_ACK_GEN_2, workq_ACK_GEN_2, WORKQ_TYPE_ACK_GEN, WORKQ_SIZE_ACK_GEN, 8);
	wait_global_start_();
	for (;;) {
		__event___handler_ACK_GEN_ack_gen_2();
	}
}
