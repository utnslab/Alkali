...
void ASTDumper::dump(ExprAST *expr) {
  llvm::TypeSwitch<ExprAST *>(expr)
      .Case<BinaryExprAST, CallExprAST, LiteralExprAST, NumberExprAST,
            ReturnExprAST, StructLiteralExprAST, VarDeclExprAST,
            VariableExprAST, PathExprAST, InitExprAST>([&](auto *node) { this->dump(node); })
      .Default([&](ExprAST *) {
...
/// @brief Print a path expression
/// @param node 
void ASTDumper::dump(PathExprAST *node) {
  INDENT();

  llvm::errs() << "path: " << node->print() << "\n";
}
...
void ASTDumper::dump(CallExprAST *node) {
  INDENT();
  llvm::errs() << "Call '" << node->printCallee() << "' [ " << loc(node) << "\n";
...
/// @brief Dummp an initialization process
/// @param node 
void ASTDumper::dump(InitExprAST *node) {
  INDENT();
  llvm::errs() << "Init \n";
  dump(node->getType());
  indent();
  for (auto &value : node->getInitVals())
    dump(value.get());
}
